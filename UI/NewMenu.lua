...
local OldPreloading; OldPreloading = hookmetamethod(game, '__namecall', newcclosure(function(self, ...) -- Adding PreloadAsync bypass?
    ...
    if not checkcaller() and self == ProxyContentProvider then
        if (Method == 'PreloadAsync' or Method == 'preloadAsync') then
            ...
            if typeof(PreloadTable) == 'table' then
                ...
                for _, Index in ipairs(PreloadTable) do
                    if typeof(Index) == 'Instance' and (Index == ProxyCoreGui or Index:IsDescendantOf(ProxyCoreGui)) then
                        CoreGuiFound = true;
                    else
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        local HueTextSize = Library:GetTextBounds('Hex color', Font, 16) + 3
        local RgbTextSize = Library:GetTextBounds('255, 255, 255', Font, 16) + 3
...
do
    ...
    function Funcs:AddLabel(Text, DoesWrap)
        ...
        if DoesWrap then
            local Y = select(2, Library:GetTextBounds(Text, Font, 14, Vector2.new(TextLabel.AbsoluteSize.X, math.huge)))
...
function Library:SetWatermark(Text)
    local X, Y = Library:GetTextBounds(Text, Font, 14);
...
function Library:Notify(Text, Time)
    local XSize, YSize = Library:GetTextBounds(Text, Font, 14);
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(550, 607) end
    ...
    function Window:AddTab(Name)
        ...
        local TabButtonWidth = Library:GetTextBounds(Name, Font, 16);
