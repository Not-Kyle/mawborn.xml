...
function String.valueOf(Source)
	return String.new(tostring(Source));
end
...
function String.append(Source1, Source2)
	local self = Source1;
	local addToSelf = Source2;

	local A = checkString(self);
	local B = checkString(addToSelf);
...
function String:getValue()
    if type(self._Value) == 'table' then
        return tostring(self._Value._Value or '');
    end

    return tostring(self._Value or '');
end
...
function String:concatWithTable(Separator: string)
    assert(type(self._Value) == 'table', 'String:concatWithTable expects _Value to be a table');
    
    self._Value = table.concat(self._Value, Separator);
    return self;
end

function String:concat(Source)
	return String.append(self, Source);
end

return TableFreeze(String);
